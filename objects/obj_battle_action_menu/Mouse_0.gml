...
if ( mouse_x < 0.5*menu_width && mouse_x > 1 ) {
	// mouse_y = yy + 1 + button_height*(i)/2
	selected_index = floor(2*(mouse_y - yy - 1)/button_height);
	show_debug_message("left col!");
}

// right_column
if ( mouse_x < menu_width && mouse_x > 0.5*menu_width ) {
	// mouse_y = yy + 1 + button_height*(i - 1)/2
	selected_index = floor(1 + 2*(mouse_y - yy - 1)/button_height);
